...
with tab_metrics:
    ...
    if metrics is None:
        ...
        col1.metric(label="🖥️ CPU", value=f"{cpu:.1f} %",
                    help="Utilisation CPU instantanée")
        col2.metric(label="🧠 Mémoire", value=f"{mem:.1f} %",
                    help=f"{metrics.get('memory_used_gb', 0):.1f} GB / "
                         f"{metrics.get('memory_total_gb', 0):.1f} GB")
        col3.metric(label="💾 Disque", value=f"{disk:.1f} %",
                    help=f"{metrics.get('disk_used_gb', 0):.1f} GB / "
        ...
        with st.expander("Détails mémoire & disque"):
            ...
            with dcol1:
                st.write("**Mémoire**")
                st.write(f"- Utilisée : {metrics.get('memory_used_gb', 0):.2f} GB")
                st.write(f"- Totale   : {metrics.get('memory_total_gb', 0):.2f} GB")
            with dcol2:
                st.write("**Disque**")
                st.write(f"- Utilisé : {metrics.get('disk_used_gb', 0):.2f} GB")
                st.write(f"- Total   : {metrics.get('disk_total_gb', 0):.2f} GB")
...
with tab_servers:
    ...
    if servers_list:
        ...
        df_servers = pd.DataFrame([
            {
                "Statut": STATUS_COLORS.get(s["status"], "⚪") + " " + s["status"],
                "Nom": s["name"],
                "Host": s["host"],
                "Port": s["port"],
                "URL": s.get("base_url", ""),
        ...
        st.dataframe(df_servers.drop(columns=["ID"]),
                     use_container_width=True, hide_index=True)

        st.markdown("#### Actions")
        action_col1, action_col2 = st.columns(2)
        with action_col1:
            server_names = {s["name"]: s["id"] for s in servers_list}
            selected_name = st.selectbox(
                "Sélectionner un serveur", options=list(server_names.keys()),
                key="select_server",
            )
        with action_col2:
            st.write("")
            ...
            if selected_name:
                ...
                if btn_col1.button("🔍 Health Check"):
                    ...
                    if result:
                        st.success(f"Statut mis à jour : {result['status']}")
                    else:
    ...
    with st.form("register_server_form", clear_on_submit=True):
        ...
        with f_col1:
            f_name = st.text_input("Nom du serveur", placeholder="prod-api-01")
        with f_col2:
            f_host = st.text_input("Host / IP", placeholder="192.168.1.100")
        with f_col3:
            f_port = st.number_input("Port", min_value=1, max_value=65535, value=8000)
